...
def load_scene(scene_dir: Path) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    rgb = np.asarray(Image.open(scene_dir / "rgb.jpg").convert("RGB"))
...
def sample_points(points: np.ndarray, n:int = 1024) -> np.ndarray:
    """
    Randomly scene or pad points to match exactly n points
    """
    num_points = points.shape[0]
    if num_points == 0:
        raise ValueError("No points available to scene from")
...
def normalize_points(points: np.ndarray) -> tuple[np.ndarray, float]:
    """
    Scale centered points to fit within a unit sphere.
    Returns scaled points and the scale factor (needed to reconstruct real-world coords).
    """
